...
namespace MyMachineLearningLibrary
{
	...
	{
		public NeuralNetMatrix[] M { get; set; }
		public NeuralNetMatrix[] V { get; set; }
		...
		{
			M = new NeuralNetMatrix[neuralNetwork.Layers.Count - 1];
			V = new NeuralNetMatrix[neuralNetwork.Layers.Count - 1];

			for (int i = 1; i < neuralNetwork.Layers.Count; i++) // Do not include the input layer
			{
				M[i - 1] = new NeuralNetMatrix(neuralNetwork.Layers[i].Gradients.RowLength, neuralNetwork.Layers[i].Gradients.ColoumnLength);
				V[i - 1] = new NeuralNetMatrix(neuralNetwork.Layers[i].Gradients.RowLength, neuralNetwork.Layers[i].Gradients.ColoumnLength);
			}
		}
